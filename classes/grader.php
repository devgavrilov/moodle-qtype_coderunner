...
abstract class qtype_coderunner_grader {
    ...
    static public function available_graders() {
        return array('EqualityGrader'       => 'qtype_coderunner_equality_grader',
                     ...
                     'TestLibGrader'        => 'qtype_coderunner_testlib_grader'
                );
    }

    /** Called to grade the output generated by a student's code for
     *  a given testcase of question. Returns a single TestResult object, which
     *  must have called tidy on the expected, output and stdin fields
     *  to limit their lengths.
     */
    public function grade(&$output, &$testcase, qtype_coderunner_question $question, $isbad = false) {
        if ($isbad) {
            $outcome = new qtype_coderunner_test_result($testcase, false, 0.0, $output);
        } else {
            $outcome = $this->grade_known_good($output, $testcase, $question);
        }
        return $outcome;
    }


    abstract protected function grade_known_good(&$output, &$testcase, qtype_coderunner_question $question);
